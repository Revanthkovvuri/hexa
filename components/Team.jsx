...
function AdvisorCard({ advisor }) {
  return (
    <div
      className="group relative flex-shrink-0 w-[140px] h-[215px] md:w-[220px] md:h-[320px] rounded-xl overflow-hidden flex flex-col
        border border-[#D9B24C]/20
        bg-black
        ...
        md:hover:border-[#D9B24C]/35
        cursor-default select-none mx-auto touch-manipulation"
    >
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[#D9B24C]/35 to-transparent z-10 pointer-events-none" />

      <div className="relative w-full h-[135px] md:h-[205px] overflow-hidden bg-black/60 flex-shrink-0">
        <img
          src={advisor.photo_url || PLACEHOLDER_SVG}
          alt={advisor.name}
        ...
        <div className="absolute top-2 left-2 flex items-center gap-1 bg-[#42AACC] text-white px-1.5 py-[2px] md:px-2 md:py-[3px] rounded-md text-[8px] md:text-[9px] font-black tracking-widest uppercase z-10">
      ...
      <div className="flex-1 flex flex-col justify-center items-center px-2 py-2 md:px-4 md:py-3 text-center">
        <p className="text-[12px] md:text-[16px] font-bold text-white leading-tight break-words" title={advisor.name}>
          {advisor.name || 'TBA'}
        </p>
        {advisor.role && (
          <p className="text-[9px] md:text-[11px] text-[#42AACC]/80 font-medium mt-1 leading-snug break-words" title={advisor.role}>
            {advisor.role}
          </p>
        )}
        {advisor.department && (
          <p className="text-[8px] md:text-[10px] text-white/40 font-medium mt-0.5 leading-snug break-words" title={advisor.department}>
            {advisor.department}
          </p>
        )}
      </div>
...
export default function Team() {
  ...
  useEffect(() => {
    async function fetchTeamData() {
      ...
      try {
        const [domainsRes, leadsRes, membersRes, mentorsRes, captainsRes, advisorsRes] = await Promise.all([
          ...
          supabase.from('advisors').select('*').order('display_order'),
        ]);
  ...
  return (
    <section className="relative py-20 md:py-28 px-6 md:px-10 max-w-[1440px] mx-auto overflow-hidden" id="team">
      ...
      {/* Advisors Row - Shown first, before Mentors */}
      {!loading && advisors.length > 0 && (
        <div className="relative z-10 mb-10">
          <div className="flex flex-col items-center mb-5">
            <div className="flex items-center gap-3">
              <div className="w-2 h-2 rounded-full flex-shrink-0 bg-[#42AACC]" />
              <span className="text-xs font-black tracking-[0.18em] uppercase text-[#42AACC]">Faculty Advisors</span>
              <div className="hidden sm:block h-[1px] w-20 bg-gradient-to-r from-[#42AACC]/30 to-transparent" />
            </div>
          </div>

          <div className="flex justify-center gap-3 md:gap-5 overflow-x-auto pb-2 no-scrollbar">
            {advisors.map((advisor) => (
              <div key={advisor.id} className="flex-shrink-0">
          ...
          <div className="mt-8 h-[1px] bg-gradient-to-r from-[#42AACC]/15 via-white/[0.06] to-transparent" />
        </div>
      )}
