...
function RoadmapSettings({ toast }) {
  ...
  const fetchData = async () => {
    ...
    setActiveCheckpointId(settingsRes.data?.value || null);
    setLoading(false);
  };

  const saveActiveCheckpoint = async (checkpointId) => {
    setSaving(true);
    const { error } = await supabase
      .from('site_settings')
      .update({ value: checkpointId })
      .eq('key', 'roadmap_active_checkpoint');
    
    if (error) { toast(error.message, 'error'); setSaving(false); return; }
    
    // Auto-sync status: set selected as 'current', others as 'completed' or 'locked'
    const checkpoint = checkpoints.find(c => c.id === checkpointId);
    if (checkpoint) {
      const updates = checkpoints.map(c => ({
        id: c.id,
        status: c.id === checkpointId ? 'current' : (c.status === 'current' ? 'locked' : c.status),
      }));
      
      for (const update of updates) {
        await supabase.from('roadmap_checkpoints').update({ status: update.status }).eq('id', update.id);
      }
    ...
    setSaving(false);
    toast('Active checkpoint updated!');
  };

  const addCheckpoint = async () => {
    if (!newCheckpoint.phase || !newCheckpoint.name) {
      toast('Phase and name required.', 'error');
      return;
    }
    
    setSaving(true);
    const id = `phase-${newCheckpoint.phase}`;
    const tDecimal = newCheckpoint.t / 100;
    
    const { error } = await supabase.from('roadmap_checkpoints').insert({
      id,
      phase: newCheckpoint.phase,
      name: newCheckpoint.name,
      description: newCheckpoint.description,
      status: 'locked',
      status_label: newCheckpoint.status_label || 'UPCOMING',
      t: tDecimal,
      display_order: checkpoints.length + 1,
    });
    ...
    setAdding(false);
    setNewCheckpoint({ phase: '', name: '', description: '', t: 50, status: 'locked', status_label: '' });
    fetchData();
  };

  const updateCheckpoint = async (id, updates) => {
    setSaving(true);
    const { error } = await supabase.from('roadmap_checkpoints').update(updates).eq('id', id);
  ...
  const deleteCheckpoint = async (id) => {
    ...
    const { error } = await supabase.from('roadmap_checkpoints').delete().eq('id', id);
    if (error) { toast(error.message, 'error'); return; }
    
    toast('Checkpoint deleted.', 'info');
    if (activeCheckpointId === id) {
  ...
  const startEdit = (checkpoint) => {
    ...
    setEditForm({
      ...
      t: Math.round(checkpoint.t * 100),
      status: checkpoint.status,
      status_label: checkpoint.status_label,
    });
  };

  const saveEdit = async () => {
    if (!editForm.phase || !editForm.name) {
      toast('Phase and name required.', 'error');
      return;
    }
    
    const tDecimal = editForm.t / 100;
    await updateCheckpoint(editingId, {
      ...
      status_label: editForm.status_label,
    });
  };
  ...
  return (
    <div>
      <PageHeader title="Roadmap" subtitle="Manage checkpoint positions and active state" compact />
      
      <Card style={{ marginBottom: 24 }}>
        <SectionTitle>Active Checkpoint (Car Position)</SectionTitle>
        <div style={{ color: '#fff', fontSize: 13, marginBottom: 12 }}>
          Current: {checkpoints.find(c => c.id === activeCheckpointId)?.name || 'None selected'}
          {activeCheckpointId && (
            <span style={{ color: C.muted, marginLeft: 8 }}>
              ({Math.round(checkpoints.find(c => c.id === activeCheckpointId)?.t * 100 || 0)}%)
            </span>
          )}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {checkpoints.map(cp => (
            <div
              key={cp.id}
              onClick={() => saveActiveCheckpoint(cp.id)}
              style={{
                padding: '10px 14px',
                borderRadius: 8,
                cursor: 'pointer',
                background: activeCheckpointId === cp.id ? 'rgba(255,198,0,0.1)' : C.navy,
                border: `1px solid ${activeCheckpointId === cp.id ? C.gold : 'rgba(255,255,255,0.06)'}`,
                color: activeCheckpointId === cp.id ? C.gold : C.muted,
                fontSize: 12,
                fontWeight: activeCheckpointId === cp.id ? 700 : 500,
                transition: 'all 0.15s',
              }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <span style={{ marginRight: 8 }}>Phase {cp.phase}:</span>
                  <span style={{ color: '#fff' }}>{cp.name}</span>
                </div>
        ...
        {saving && <div style={{ color: C.teal, fontSize: 11, marginTop: 8 }}>Saving…</div>}
      </Card>

      <Card>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <SectionTitle>All Checkpoints ({checkpoints.length})</SectionTitle>
          <Btn variant="secondary" onClick={() => setAdding(true)} style={{ fontSize: 11 }}>+ ADD CHECKPOINT</Btn>
        </div>

        <AnimatePresence>
          {adding && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <div style={{ background: C.navy, borderRadius: 10, padding: 16, marginBottom: 16, border: `1px solid rgba(0,200,224,0.15)` }}>
                <SectionTitle>New Checkpoint</SectionTitle>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                  <Input label="PHASE NUMBER" value={newCheckpoint.phase} onChange={e => setNewCheckpoint(p => ({ ...p, phase: e.target.value }))} placeholder="e.g. 5" />
                  <Input label="NAME" value={newCheckpoint.name} onChange={e => setNewCheckpoint(p => ({ ...p, name: e.target.value }))} placeholder="e.g. SCALING" />
                </div>
                <div style={{ marginBottom: 10 }}>
                  <Input label="DESCRIPTION" value={newCheckpoint.description} onChange={e => setNewCheckpoint(p => ({ ...p, description: e.target.value }))} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                  <Input label="POSITION (%)" type="number" min="0" max="100" value={newCheckpoint.t} onChange={e => setNewCheckpoint(p => ({ ...p, t: parseInt(e.target.value) || 0 }))} />
                  <div>
                    <label style={{ color: C.muted, fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>STATUS</label>
                    <select
                      value={newCheckpoint.status}
                      onChange={e => setNewCheckpoint(p => ({ ...p, status: e.target.value }))}
                ...
                <div style={{ marginBottom: 12 }}>
                  <Input label="STATUS LABEL" value={newCheckpoint.status_label} onChange={e => setNewCheckpoint(p => ({ ...p, status_label: e.target.value }))} placeholder="e.g. UPCOMING / Q2 2025" />
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <Btn onClick={addCheckpoint} disabled={saving}>{saving ? 'Adding…' : 'Add Checkpoint'}</Btn>
                  <Btn variant="ghost" onClick={() => setAdding(false)}>Cancel</Btn>
                </div>
              </div>
            </motion.div>
              ...
              {editingId === cp.id ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    <Input label="PHASE" value={editForm.phase} onChange={e => setEditForm(p => ({ ...p, phase: e.target.value }))} />
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    <Input label="POSITION (%)" type="number" min="0" max="100" value={editForm.t} onChange={e => setEditForm(p => ({ ...p, t: parseInt(e.target.value) || 0 }))} />
                    <div>
                      <label style={{ color: C.muted, fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>STATUS</label>
                      <select
                  ...
                  <Input label="STATUS LABEL" value={editForm.status_label} onChange={e => setEditForm(p => ({ ...p, status_label: e.target.value }))} />
                  <div style={{ display: 'flex', gap: 6 }}>
                    <Btn onClick={saveEdit} disabled={saving} style={{ flex: 1, fontSize: 11 }}>Save</Btn>
                    <Btn variant="ghost" onClick={() => { setEditingId(null); setEditForm({}); }} style={{ fontSize: 11 }}>✕</Btn>
                  </div>
                </div>
              ) : (
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 6 }}>
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 8, fontSize: 10 }}>
                        <span style={{ color: C.teal }}>{Math.round(cp.t * 100)}%</span>
                        <span style={{
                          color: cp.status === 'completed' ? C.green : cp.status === 'current' ? C.gold : C.muted,
                          textTransform: 'uppercase',
                          fontWeight: 700,
                        }}>
                          {cp.status}
                        </span>
                        <span style={{ color: C.muted }}>{cp.status_label}</span>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <Btn variant="secondary" onClick={() => startEdit(cp)} style={{ fontSize: 10 }}>Edit</Btn>
                      <Btn variant="danger" onClick={() => deleteCheckpoint(cp.id)} style={{ fontSize: 10 }}>✕</Btn>
                    </div>
                  </div>
                </>
              )}
        ...
        {checkpoints.length === 0 && (
          <div style={{ color: C.muted, fontSize: 13, textAlign: 'center', padding: 40 }}>
            No checkpoints yet. Add your first checkpoint to get started.
          </div>
        )}
      </Card>
    </div>
...
export default function AdminPage() {
  ...
  const SECTIONS = {
    ...
    roadmap:   <RoadmapSettings toast={toast} />,
  };
